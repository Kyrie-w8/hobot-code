...
- root 下的 `bash`、`write`、`edit` 始终逐次确认；非交互 root 会话拒绝这些调用。
...
安全修复以当前主分支为准。发布新包后应尽快升级，并在部署前执行 `make check` 与板端验证。回滚要求 root 权限和完整的前一版本备份；首次安装通常没有可用回滚点。

Pi、`fd` 与 `ripgrep` 发行产物按锁文件中的 SHA256 校验。校验只证明下载内容与锁定内容一致，不替代对上游代码、许可证或供应链的评估。
