...
Hobot Code 可以读取项目、执行命令并调用板端设备。它应被视为拥有当前 OS 用户权限的开发工具，而不是隔离不可信代码的沙箱。
...
这些机制不是通用进程隔离。获得确认的 Shell 命令，以及第三方扩展、Hook 或语言服务器，仍可访问当前用户能够访问的文件、网络和设备。高价值环境应再使用独立账号、文件权限、容器或系统级强制访问控制。
...
`agentd` 按 OS 用户运行，只监听权限为 `0600` 的本机 Unix socket，所在目录权限为 `0700`；Linux 发行版还核对连接方 UID。它不是跨用户安全沙箱：已经取得同一用户权限的进程可以访问该用户的任务状态，也可以直接运行 Hobot Code。
