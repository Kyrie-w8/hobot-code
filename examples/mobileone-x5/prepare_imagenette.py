#!/usr/bin/env python3
"""Prepare balanced Imagenette calibration and evaluation data for MobileOne."""
...
def balanced_samples(root: Path, count_per_class: int, seed: int) -> list[Path]:
    ...
    for offset, synset in enumerate(sorted(IMAGENET_CLASS_INDEX)):
        candidates = sorted(
            path for path in (root / synset).iterdir() if path.suffix.lower() in {".jpeg", ".jpg", ".png"}
        )
...
def main() -> None:
    ...
    manifest = {
        ...
        "preprocessing": "RGB; resize shorter edge to 256; center crop 224; model-embedded ImageNet normalization",
        "seed": args.seed,
