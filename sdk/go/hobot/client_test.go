...
func TestClientReusesControlBridgeAndDecodesTasks(t *testing.T) {
	...
	if err != nil || snapshot.BoardID != "s100" || len(snapshot.BPUDevices) != 2 || len(snapshot.BPUCores) != 1 || snapshot.BPUCores[0].UtilizationPercent != 42 || snapshot.AIMemory.BPUAllocatedBytes != 33554432 || snapshot.Memory.AvailableBytes == 0 || len(snapshot.ThermalZones) != 1 || snapshot.ThermalZones[0].Celsius != 52.5 {
