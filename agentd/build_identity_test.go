...
func validBuildInfo(t *testing.T, version, binarySHA256 string) []byte {
	t.Helper()
	dirty := false
...
func TestReadBuildIdentityBindsMetadataToBinary(t *testing.T) {
	...
	if err := os.WriteFile(metadata, validBuildInfo(t, "0.26.0", binarySHA256), 0o644); err != nil {
		t.Fatal(err)
	}
	identity := readBuildIdentity(binary, metadata, "0.26.0")
	if identity.Status != "verified" || identity.Commit != strings.Repeat("a", 40) || identity.BinarySHA256 == "" || identity.Dirty == nil || *identity.Dirty || identity.PiVersion != "0.84.1" {
...
func TestReadBuildIdentityRejectsUntrustedMetadata(t *testing.T) {
	...
	if err := os.WriteFile(metadata, validBuildInfo(t, "0.25.0", binarySHA256), 0o644); err != nil {
		t.Fatal(err)
	}
