...
const mockBackend: Backend = {
  ...
  ListExtensions: async (): Promise<ExtensionCatalog> => ({schemaVersion: 1, apiVersion: 'hobot.extensions/v1', productVersion: '0.26.0', hostVersion: '0.26.0', entries: [{id: 'hobot.rdk-core', name: 'RDK development core', version: '0.26.0', kind: 'extension', description: 'D-Robotics models, tools, knowledge, safety, and board coordination.', origin: 'built-in', scope: 'system', runtime: 'pi-extension', entrypoint: 'rdk/index.ts', trust: 'product', defaultEnabled: true, required: true, provides: ['provider.drobotics'], requires: ['pi.extension-api'], permissions: ['workspace', 'subprocess', 'rdk-devices'], targets: ['x5', 's100', 's600']}], policy: {inventoryOnly: true, executionAuthority: 'pi-runtime', permissionAuthority: 'board', thirdPartyRuntime: 'current-user', hotReload: false}}),
