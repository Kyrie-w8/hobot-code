import type {Board, Connection, DeploymentInspection, DeploymentStatus, EventEnvelope, EventPage, ForkTaskRequest, ImageContent, ModelOption, StartDeploymentRequest, SupportBundle, SystemSnapshot, Task, TaskPage, WorkspaceListing} from './types';
...
const mockBoard: Board = {id: 's600-demo', name: 'RDK S600', host: '10.112.10.106', user: 'root', port: 22};
const now = new Date();
...
const mockBackend: Backend = {
  ...
  ConnectBoard: async (id: string): Promise<Connection> => ({board: {...mockBoard, id}, connected: true, daemon: {version: '0.24.0', pid: 834124, startedAt: now.toISOString(), activeTasks: 3, maximumTasks: 3, stateRoot: '/root/.local/state/hobot-code'}, capabilities: {protocolMin: 1, protocolMax: 1, eventSchema: 3, capabilities: ['events.normalized.v3', 'system.snapshot', 'support.bundle.v1', 'deployments.v1', 'tasks.lifecycle', 'tasks.page', 'events.page', 'tasks.resume', 'tasks.restart', 'tasks.fork', 'tasks.models', 'tasks.permissions', 'tasks.images', 'models.capabilities.v1', 'workspaces.browse', 'bridge.stdio'], maximumActiveTasks: 3, maximumRetainedTasks: 100}, compatibility: {status: 'supported', summary: 'Board and Studio capabilities are compatible.', appVersion: '0.24.0', agentdVersion: '0.24.0', protocol: 1, eventSchema: 3, boardId: 's600', rdkOsVersion: '5.1.0', validatedTarget: true, issues: []}}),
  RefreshBoard: async (id: string): Promise<Connection> => ({board: {...mockBoard, id}, connected: true, daemon: {version: '0.23.1', pid: 834124, startedAt: now.toISOString(), activeTasks: 3, maximumTasks: 3, stateRoot: '/root/.local/state/hobot-code'}, capabilities: {protocolMin: 1, protocolMax: 1, eventSchema: 3, capabilities: ['events.normalized.v3', 'system.snapshot', 'tasks.lifecycle', 'tasks.page', 'events.page'], maximumActiveTasks: 3, maximumRetainedTasks: 100}, compatibility: {status: 'limited', summary: 'Connected with limited features.', appVersion: '0.24.0', agentdVersion: '0.23.1', protocol: 1, eventSchema: 3, boardId: 's600', rdkOsVersion: '5.1.0', validatedTarget: true, issues: [{code: 'version-line-mismatch', severity: 'warning', message: 'Studio and board service are from different release lines.', action: 'Update Hobot Code on the board.'}]}}),
