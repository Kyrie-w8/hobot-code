import type {Board, BoardUpdateCheck, BoardUpdateResult, Connection, DeploymentInspection, DeploymentStatus, EventEnvelope, EventPage, ExtensionCatalog, ForkTaskRequest, ImageContent, ModelConformance, ModelHealth, ModelOption, StartDeploymentRequest, SupportBundle, SystemSnapshot, Task, TaskPage, WorkspaceApplyResult, WorkspaceChanges, WorkspaceDelivery, WorkspaceIsolation, WorkspaceListing} from './types';
...
const mockBackend: Backend = {
  GetAppVersion: async () => '0.26.0',
  ListBoards: async () => [mockBoard],
  ProbeBoard: async (board: Board): Promise<Connection> => ({board, connected: true, daemon: {version: '0.26.0', pid: 834124, startedAt: now.toISOString(), activeTasks: 3, maximumTasks: 3, stateRoot: '/root/.local/state/hobot-code'}, capabilities: {protocolMin: 1, protocolMax: 1, eventSchema: 4, capabilities: ['extensions.catalog.v1', 'events.normalized.v3', 'events.normalized.v4', 'events.items.v1', 'tasks.queue.v1', 'tasks.failure.v1', 'workspaces.changes.v1', 'workspaces.isolation.v1', 'workspaces.write-leases.v1', 'system.snapshot', 'tasks.lifecycle', 'tasks.page', 'events.page'], maximumActiveTasks: 3, maximumRetainedTasks: 100}, snapshot: {capturedAt: now.toISOString(), board: 'D-Robotics RDK S600', boardId: 's600', hostname: 'drobot', rdkOsVersion: '5.1.0', kernel: '6.1.158-rt58', architecture: 'arm64', cpuCores: 18, loadAverage: [], memory: {totalBytes: 0, availableBytes: 0}, disk: {path: '/root', totalBytes: 0, availableBytes: 0}, thermalZones: [], bpuDevices: [], rdkUtilities: {}, uptimeSeconds: 0}, compatibility: {status: 'supported', summary: 'Board and Studio capabilities are compatible.', appVersion: '0.26.0', agentdVersion: '0.26.0', protocol: 1, eventSchema: 4, boardId: 's600', rdkOsVersion: '5.1.0', validatedTarget: true, issues: []}}),
  SaveBoard: async (board: Board) => ({...board, id: board.id || `board-${Date.now()}`}),
  RemoveBoard: async () => undefined,
  ConnectBoard: async (id: string): Promise<Connection> => ({board: {...mockBoard, id}, connected: true, daemon: {version: '0.26.0', pid: 834124, startedAt: now.toISOString(), activeTasks: 3, maximumTasks: 3, stateRoot: '/root/.local/state/hobot-code'}, capabilities: {protocolMin: 1, protocolMax: 1, eventSchema: 4, capabilities: ['extensions.catalog.v1', 'events.normalized.v3', 'events.normalized.v4', 'events.items.v1', 'tasks.queue.v1', 'tasks.failure.v1', 'workspaces.changes.v1', 'workspaces.isolation.v1', 'workspaces.write-leases.v1', 'workspaces.delivery.v1', 'system.snapshot', 'support.bundle.v1', 'deployments.v1', 'tasks.lifecycle', 'tasks.page', 'events.page', 'tasks.resume', 'tasks.restart', 'tasks.fork', 'tasks.fork.deferred-prompt.v1', 'tasks.models', 'tasks.permissions', 'tasks.images', 'models.capabilities.v1', 'models.health.v1', 'models.conformance.v1', 'workspaces.browse', 'bridge.stdio'], maximumActiveTasks: 3, maximumRetainedTasks: 100}, compatibility: {status: 'supported', summary: 'Board and Studio capabilities are compatible.', appVersion: '0.26.0', agentdVersion: '0.26.0', protocol: 1, eventSchema: 4, boardId: 's600', rdkOsVersion: '5.1.0', validatedTarget: true, issues: []}}),
  RefreshBoard: async (id: string): Promise<Connection> => ({board: {...mockBoard, id}, connected: true, daemon: {version: '0.25.0', pid: 834124, startedAt: now.toISOString(), activeTasks: 3, maximumTasks: 3, stateRoot: '/root/.local/state/hobot-code'}, capabilities: {protocolMin: 1, protocolMax: 1, eventSchema: 3, capabilities: ['events.normalized.v3', 'system.snapshot', 'tasks.lifecycle', 'tasks.page', 'events.page'], maximumActiveTasks: 3, maximumRetainedTasks: 100}, compatibility: {status: 'limited', summary: 'Connected with limited features.', appVersion: '0.26.0', agentdVersion: '0.25.0', protocol: 1, eventSchema: 3, boardId: 's600', rdkOsVersion: '5.1.0', validatedTarget: true, issues: [{code: 'version-line-mismatch', severity: 'warning', message: 'Studio and board service are from different release lines.', action: 'Update Hobot Code on the board.'}]}}),
  CheckBoardUpdate: async (): Promise<BoardUpdateCheck> => ({status: 'available', installedVersion: '0.26.0', availableVersion: '0.27.0', message: 'A verified stable update is available.'}),
  InstallBoardUpdate: async (id: string): Promise<BoardUpdateResult> => ({changed: true, previousVersion: '0.26.0', installedVersion: '0.27.0', message: 'The board update was verified and Studio reconnected.', connection: await mockBackend.ConnectBoard(id)}),
  ...
  ListExtensions: async (): Promise<ExtensionCatalog> => ({
    schemaVersion: 1, apiVersion: 'hobot.extensions/v1', productVersion: '0.26.0', hostVersion: '0.26.0', capturedAt: new Date().toISOString(),
    entries: [
      {id: 'hobot.rdk-core', name: 'RDK development core', version: '0.26.0', kind: 'extension', description: 'D-Robotics models, tools, knowledge, safety, and board coordination.', origin: 'built-in', scope: 'system', runtime: 'pi-extension', entrypoint: 'rdk/index.ts', trust: 'product', defaultEnabled: true, required: true, provides: ['provider.drobotics'], requires: ['pi.extension-api'], permissions: ['workspace', 'subprocess', 'rdk-devices'], targets: ['x5', 's100', 's600'], status: 'included'},
      {id: 'hobot.skill.board', name: 'RDK board development', version: '0.26.0', kind: 'skill', description: 'Board discovery, BPU workflows, deployment checks, and official RDK knowledge.', origin: 'built-in', scope: 'system', runtime: 'pi-skill', entrypoint: 'skills/rdk-board/SKILL.md', trust: 'product', defaultEnabled: true, required: false, provides: ['skill.rdk-board'], requires: ['hobot.rdk-core'], permissions: ['workspace', 'subprocess', 'rdk-devices'], targets: ['x5', 's100', 's600'], status: 'included'},
...
export const api = {
  ...
  startTask: (boardId: string, request: {name: string; cwd: string; prompt: string; images?: ImageContent[]; approve: boolean; model?: string; permissionMode?: string; workspaceMode?: 'shared' | 'worktree'; sandboxMode?: 'review' | 'workspace' | 'system' | 'off'}): Promise<Task> => backend().StartTask(boardId, request),
  ...
  extensions: (boardId: string): Promise<ExtensionCatalog> => backend().ListExtensions(boardId),
