...
const mockBackend: Backend = {
  ...
  CheckBoardUpdate: async (): Promise<BoardUpdateCheck> => ({status: 'available', installedVersion: '0.26.0', availableVersion: '0.27.6', message: 'A verified stable update is available.'}),
	InstallBoardUpdate: async (id: string): Promise<BoardUpdateResult> => ({changed: true, previousVersion: '0.26.0', installedVersion: '0.27.6', message: 'The board update was verified and Studio reconnected.', connection: await mockBackend.ConnectBoard(id)}),
  ...
  ListExtensions: async (): Promise<ExtensionCatalog> => ({
    schemaVersion: 1, apiVersion: 'hobot.extensions/v1', productVersion: '0.27.6', hostVersion: '0.27.6', capturedAt: new Date().toISOString(),
    entries: [
      {id: 'hobot.rdk-core', name: 'RDK development core', version: '0.27.6', kind: 'extension', description: 'D-Robotics models, tools, knowledge, safety, and board coordination.', origin: 'built-in', scope: 'system', runtime: 'pi-extension', entrypoint: 'rdk/index.ts', trust: 'product', defaultEnabled: true, required: true, provides: ['provider.drobotics'], requires: ['pi.extension-api'], permissions: ['workspace', 'subprocess', 'rdk-devices'], targets: ['x5', 's100', 's600'], status: 'included'},
      {id: 'hobot.skill.board', name: 'RDK board development', version: '0.27.6', kind: 'skill', description: 'Board discovery, BPU workflows, deployment checks, and official RDK knowledge.', origin: 'built-in', scope: 'system', runtime: 'pi-skill', entrypoint: 'skills/rdk-board/SKILL.md', trust: 'product', defaultEnabled: true, required: false, provides: ['skill.rdk-board'], requires: ['hobot.rdk-core'], permissions: ['workspace', 'subprocess', 'rdk-devices'], targets: ['x5', 's100', 's600'], status: 'included'},
