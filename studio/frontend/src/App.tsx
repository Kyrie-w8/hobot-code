...
function AccessSettingsDialog({permissionMode, sandboxMode, networkMode, workspaceMode, summary, hasAutoReview, hasWorkspace, workspaceEligible, workspaceLoading, workspaceReason, hasSandbox, hasNetwork, sandboxAvailable, networkModes, modelOnly, canChangePermissions, canChangeSandbox, canChangeNetwork, canStopWorker, busy, onWorkspace, onPermission, onSandbox, onNetwork, onStopWorker, onClose}: {
  ...
  const autoReviewSandboxEligible = sandboxAvailable && ['review', 'workspace'].includes(sandboxMode);
  return <div className="modal-backdrop"><section className="modal access-settings-modal" role="dialog" aria-modal="true" aria-labelledby="access-settings-title">
    ...
    <div className="form-grid access-settings-grid">
      ...
      <label><span>Approvals</span><select aria-label="Approval mode" value={permissionMode} disabled={busy || !canChangePermissions} onChange={(event) => onPermission(event.target.value)}><option value="review">Review only</option><option value="ask">Ask for changes</option>{hasAutoReview && <option value="auto-review" disabled={!autoReviewSandboxEligible} title={autoReviewSandboxEligible ? 'Board-side reviewer for eligible single actions' : 'Choose Read only or Workspace with an available OS sandbox first'}>Approve for me</option>}{permissionMode === 'auto-review' && !hasAutoReview && <option value="auto-review" disabled>Approve for me (unsupported board)</option>}<option value="developer">Developer</option></select><small>{permissionMode === 'auto-review' && !autoReviewSandboxEligible ? 'Approve for me requires an available Read only or Workspace board boundary.' : 'Approve for me is a board-side reviewer for eligible single actions; it does not expand board access.'}</small></label>
      {hasSandbox && <label><span>Board access</span><select aria-label="OS sandbox" value={sandboxMode} disabled={busy || !canChangeSandbox} onChange={(event) => onSandbox(event.target.value)}><option value="review" disabled={!sandboxAvailable}>Read only</option><option value="workspace" disabled={!sandboxAvailable}>Workspace</option><option value="system" disabled={!sandboxAvailable || permissionMode === 'auto-review'} title={permissionMode === 'auto-review' ? 'Approve for me requires Read only or Workspace' : undefined}>Board hardware</option><option value="off" disabled={networkMode !== 'shared' || permissionMode === 'auto-review'} title={permissionMode === 'auto-review' ? 'Approve for me requires Read only or Workspace' : undefined}>No sandbox</option></select><small>{permissionMode === 'auto-review' ? 'Approve for me keeps the board boundary at Read only or Workspace.' : 'Limits files, devices, and Linux capabilities.'}</small></label>}
...
function ApprovalBar({approval, busy, respond}: {approval: Approval; busy: boolean; respond: (response: Record<string, unknown>) => void}) {
  ...
  const stateLabel = {reviewing: 'Reviewing on board', approved: 'Approved by board reviewer', denied: 'Denied by board reviewer', 'manual-required': 'Manual approval required'}[view.state];
