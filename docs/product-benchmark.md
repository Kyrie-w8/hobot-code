...
本文以 2026-08-13 的 Hobot Code `0.26.0` 源码、发行锁定的 Pi `0.84.1`，以及当日可查的官方产品资料为基准。对比的目的不是复制界面，而是明确哪些能力来自上游、哪些是 Hobot Code 的产品价值、哪些差距会阻碍公开交付。
...
| 模型与 Provider | 多 Provider、自定义模型与 Provider | TUI 保留 Pi 能力并增加 D-Robotics 网关；Studio 默认只呈现已验证的 D-Robotics 模型 | 模型目录、能力和输入模态由统一协议协商 | 板端开放性已有，Studio 的 Provider 管理仍不足 |
| 扩展 | Extensions、Skills、Prompt templates、themes 和 packages | 兼容 Pi 扩展，并增加 RDK 知识、工具、Hook、LSP、记忆和目标 | Skills、Plugins、MCP、hooks、apps 有发现和管理协议 | 运行能力较强，缺少 Studio 中的可见性、启停、信任与升级管理 |
...
| 诊断与运营 | 依赖通用日志和社区排查 | 一键脱敏支持包、兼容性协商、模型健康检查、升级与回滚 | 反馈、日志、状态数据库、配置约束和多端发布体系 | 单机支持较好，缺少可选崩溃反馈、发布通道和规模化运维证据 |

## 与原生 Pi 的真实差距

Hobot Code 当前锁定的 Pi `0.84.1` 与 2026-08-13 的上游最新发行一致，因此 TUI 编辑、消息队列、会话树、自动压缩、模型选择和扩展 API 不存在版本代差。差距主要出现在产品包装，而不是 Agent 内核：

1. **Provider 管理不如 Pi 完整。** Pi 有广泛的内置 Provider、自动模型目录刷新、登录和认证预检；Hobot Code 的 TUI 继承这些能力，但 Studio 只把 D-Robotics 已验证模型做成一等入口，也没有统一的凭据、目录来源与实验模型管理页。
2. **Pi 扩展生态在 Studio 中不可见。** packages、extensions、Skills、Prompt templates 和 themes 可以运行，但普通用户难以判断加载来源、作用域、版本、信任和故障状态。
3. **上游能力的产品验收不足。** 锁版本与哈希已经做到，但每次 Pi 升级仍需形成 TUI、RPC、Side Agent、权限 Hook、compaction 和中文输入的兼容报告，不能只验证二进制能启动。
4. **模型能调用不等于 Agent 能工作。** Pi 允许快速接入 Provider；Hobot Code 还需要对每个模型验证工具参数、并行调用、错误修复、长上下文、thinking、图片与 RDK 任务完成率，才能标为“适合板端开发”。
...
5. **规模化管理。** Codex 有受管配置、可选反馈、自动化任务和多端产品面；Hobot Code 仍是单用户、单板优先，尚未形成组织策略、多板队列和可靠性数据。
...
1. **Provider 控制面。** Studio 默认展示已验证模型，同时允许用户展开板端其他 Provider；模型必须带来源、能力、健康、上下文限制和“已验证/实验性”标签，不能靠硬编码名单长期扩展。
2. **扩展控制面。** 只读、版本化的内置扩展/Skills 清单已经作为 `hobot.extensions/v1` 基线接入 CLI、agentd、SDK 与 Studio API。下一步在 Studio 统一展示 Skills、MCP、packages、hooks 和 LSP 的来源、作用域、状态与信任；安装和更新继续由板端执行并接受权限约束。
...
| 阶段 | 交付物 | 可验收结果 |
|---|---|---|
| 1. 安全基线 | 普通用户安装、RDK sandbox 能力探测、文件/网络/设备 profile | 受限 Agent 无法绕过策略读取或写入样例保护路径；降级状态可见 |
| 2. 模型适配 | Provider conformance suite、适配等级、失败诊断 | 每个公开模型有固定版本报告；工具和恢复测试全部通过才标“已验证” |
| 3. 恢复证据 | 有界 turn ledger、工具终态、工作区摘要 | 基础闭环已实现；强杀矩阵需继续验证非 Git 副作用和绝不静默重放 |
| 4. 三板可靠性 | X5/S100/S600 自动安装、升级、断网和 24 小时测试 | 发布页附带可复核矩阵，关键场景无数据丢失或静默重复执行 |
| 5. 工作流与扩展 | 只读目录已建立；继续实现逐 hunk 审阅与扩展/Skills/MCP/Provider 控制面 | 用户无需编辑 JSON 即可理解来源、能力、风险、状态并安全启停 |
