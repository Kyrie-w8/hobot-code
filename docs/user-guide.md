...
长对话首次打开时先显示最近内容。继续向上滚动会按页读取板端仍保留的更早记录，并保持当前阅读位置不跳动；阅读旧消息时，新输出只显示 **New output** 提示，不会强制拉回底部。对话右侧在至少有六条用户消息时显示消息导航轨：悬浮定位点可查看时间和两行摘要，点击会跳转并高亮原消息，上下箭头用于移动到相邻用户消息。密集定位点会分组，展开后仍可选择组内任意消息。
