...
The board remains usable, but Studio reports `limited`, when a board service is missing model capability negotiation, model route health checks, hardware telemetry, support bundles, deployment workflows, side agents, persistent task queuing, structured task failures, structured event items, rolling event-retention boundaries, workspace browsing, read-only workspace change review, isolated Git task workspaces, cross-Agent workspace write leases, or verifiable build identity. Studio labels this case **Update recommended** and states that core Agent work is still available. Event schema 2 is also accepted with a warning because it cannot expose every normalized activity detail; schema 3 remains readable, while schema 4 plus `events.items.v1` adds stable item lifecycle and bounded tool previews. `events.retention.v1` lets Studio distinguish a normally expired replay cursor from a real persistence gap; older boards remain usable but cannot make that distinction.

`support.bundle.v1` remains sufficient for the support-bundle feature itself, so an otherwise compatible older board is not rejected solely for lacking v2. `support.bundle.v2` adds board-derived status and bounded recovery findings. Studio labels v1 results as legacy, derives only a conservative summary from check counts, and never invents server-side advice.

`diagnostics.inspect.v1` separates routine, zero-side-effect readiness checks from support-file creation. `diagnostics.repair.v1` adds only explicitly confirmed, server-advertised safe repairs; older boards remain connectable but Studio marks both features unavailable. A client must never infer repairability from text or attempt to bypass the board-side active-task and path-safety checks.
